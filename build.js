...
const additions = require('./additions.json')
...
Promise.all(promises).then(() => {
  ...
  records.forEach((r) => {
    ...
    const logo = existsSync(`./logos/${logoName}`) ? logoName : undefined
    if (logo) {
  ...
  Object.keys(additions).forEach((make) => {
    Object.keys(additions[make]).forEach((model) => {
      ...
      const logoName = `${make.toLowerCase().replace(' ', '-')}.png`
      const logo = additions[make][model].logo || (existsSync(`./logos/${logoName}`) ? logoName : undefined)
      if (logo) {
        cars[make][model].logo = logo
      }
    })
  ...
  writeFileSync('./index.json', JSON.stringify(cars))
